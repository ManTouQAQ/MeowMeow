...
class RotateBlockTask(
    ...
    override fun accept(history: RegionHistory) {
        ...
        for (needMoveSnapshot in needRotateBlocks) {
            ...
            val originalSnapshot = needRotateBlocks.find { it.pos == Vector3i(newX, newY, newZ) }
            if (originalSnapshot != null){
                history.addSnapshot(newX, newY, newZ, originalSnapshot.from, needMoveSnapshot.from, originalSnapshot.fromData, needMoveSnapshot.fromData)
            }else{
                history.addSnapshot(newX, newY, newZ, block.type, needMoveSnapshot.from, block.blockData, needMoveSnapshot.fromData)
            }
